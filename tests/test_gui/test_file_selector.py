...
def test_FileSelectorWidgetC1():
    ...
    selector1 = FileSelectorWidgetC1(
        info_text="Select a file", icon_path=Path("assets\\json-icon.png").resolve()
    )
    widg_layout.addWidget(selector1.widget)

    selector2 = FileSelectorWidgetC1(
        info_text="Select a file", icon_path=Path("assets\\xlsx-icon.png").resolve()
    )
    widg_layout.addWidget(selector2.widget)

    selector3 = FileSelectorWidgetC1(
        info_text="Select a file", icon_path=Path("assets\\folder-icon.jpg").resolve()
    )
